...
## Display drivers
boot_dialog --title "Display drivers" --menu "" 20 60 6 "1" "VIRTUALBOX" "2" "VMware" "3" "INTEL" "4" "ATI" "5" "AMD" "6" "NVIDIA"
dd="$DIALOG_RESULT"
...
#AVAHI
boot_dialog --title "Avahi" --menu "Установка Avahi" 10 60 2 "1" "ДА" "2" "НЕТ"
avahiinst="$DIALOG_RESULT"

#SAMBA
boot_dialog --title "Samba" --menu "Установка Samba" 10 60 2 "1" "ДА" "2" "НЕТ"
sambainst="$DIALOG_RESULT"

if [[ "$sambainst" == "1" ]]; then
boot_dialog --title "Samba password" --passwordbox "Пожалуйста, введите пароль для user SAMBA.\n" 10 60
smb_password="$DIALOG_RESULT"
...
if [[ "$de" = "1" ]]; then
    core_packages+=' plasma kde-system kde-utilities kde-graphics kde-network packagekit-qt5 libappindicator-gtk2 libappindicator-gtk3 colord-kde kaccounts-integration kaccounts-providers latte-dock kdeconnect kdenetwork-filesharing sshfs kvantum-qt5'
    # Dolphin
    core_packages+=' dolphin dolphin-plugins kompare kdegraphics-thumbnailers kimageformats qt5-imageformats kdesdk-thumbnailers ffmpegthumbs raw-thumbnailer taglib audiocd-kio kio-gdrive'
    # Display manager
    core_packages+=' sddm'
...
core_packages+=' mesa lib32-mesa xf86-input-libinput xdg-user-dirs dhcpcd networkmanager networkmanager-openvpn network-manager-applet ppp dialog wpa_supplicant gvfs-afc gvfs-mtp exfat-utils ntfs-3g'

# Управление энергопотреблением
core_packages+=' powerdevil'

# Audio
core_packages+=' pulseaudio pulseaudio-alsa pavucontrol alsa-utils pulseaudio-equalizer-ladspa'

# Avahi install
if [[ "$avahiinst" == "1" ]]; then
    core_packages+=' avahi zeroconf-ioslave nss-mdns'
fi

# Samba install
if [[ "$sambainst" == "1" ]]; then
    core_packages+=' samba smbclient gvfs-smb smb4k'
fi
...
# Video, Audio
packages+=' ffmpeg ffmpegthumbnailer vlc deadbeef'

# General utilities/libraries
packages+=' neofetch git wget curl openssh p7zip unace unrar unzip ark mc htop gparted vim'

# Office
packages+=' libreoffice-still libreoffice-still-ru'
...
echo "##################### delete yakuake ########################"
pacman -R --noconfirm yakuake
echo "##################### delete kimagemapeditor ########################"
pacman -R --noconfirm kimagemapeditor
echo "##################### delete end ########################"
EOF
...
iptables -A INPUT -p tcp -m tcp --dport 139 -s 192.168.88.0/24 -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport 445 -s 192.168.88.0/24 -j ACCEPT
iptables-save -f /etc/iptables/iptables.rules
...
[PublicShare]
    path = /home/$username/Общедоступные
...
# Копирование скрипта inst_prog.sh на рабочий стол для установки pikaur, zsh, nVidia_optimus_manager...
echo "##################### Копирование скрипта inst_prog.sh ########################"
arch-chroot /mnt /bin/bash <<EOF
...
# ENABLE Service Samba
if [[ "$sambainst" == "1" ]]; then
echo "###################### ENABLE Service Samba s#######################"
    arch-chroot /mnt /bin/bash <<EOF
